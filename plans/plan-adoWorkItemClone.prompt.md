...
Build a PowerShell-based Azure DevOps work item cloning utility that uses REST APIs, supports source/target project cloning (including same-project), type transformation, configurable field mapping (including replaceTo value remapping), WIQL-based selection, create-or-update processing, and two-pass relation handling with fallback to original links when linked items are not cloned. The first version includes attachment cloning and outputs console summary plus JSON mapping/report artifacts.
...
11. Add replaceTo value remapping in fieldMapping rules so source values can be replaced before transform is applied.
12. When field mapping is not configured and source/target fields are incompatible, log a warning and write null/empty fallback value (based on target field type policy) instead of failing the item.
13. On initial clone, set the configured custom single-line field to the original source work item reference (source ID or URL per config policy).
14. Add update-processing logic (upsert mode): before creating a target item, query target by the configured original-source-reference field and update existing cloned item when found; create only when not found.
15. Persist source->target ID mapping in memory and incremental JSON artifact (for resumability). *Depends on 7, 9, 10, 11, 12, 13, 14.*
16. Add dry-run behavior using validateOnly mode and payload preview logs without persistence. *Parallel with 9-15 once API wrapper exists.*
17. Phase 4 - Second-pass relation and attachment reconstruction
18. Traverse original relations for each processed source item and rebuild relations on cloned/updated targets.
19. If linked work item is also cloned, remap relation URL to cloned target ID.
20. If linked work item is not cloned, apply configured fallback: default copy link to original work item URL, with optional per-relation override (skip/copy-original).
21. Rebuild hierarchy and related/dependency links in safe order and skip unsupported cross-project relation types with warnings.
22. Clone attachments (requested for v1): download source attachment content, upload to target via attachment API, then attach new URL relation to cloned work item. *Depends on 15 and API wrappers.*
23. Phase 5 - Observability, safety, and usability
24. Add structured logging (operation, item ID, API status, relation action, warnings/errors) and execution summary counters.
25. Emit console summary plus JSON output artifacts: run summary, source->target mapping, failures/skips with reasons, relation outcomes, created-vs-updated counts.
26. Add retry policy for transient API failures (429/5xx), throttle controls, and continue-on-error mode with final non-zero exit when failures exceed threshold.
27. Phase 6 - Documentation and examples
28. Document configuration, env vars, authentication setup, dry-run usage, and known limitations.
29. Provide at least one example config showing type conversion, field mapping, replaceTo value remapping, includeTypes + WIQL, link fallback behavior, null/empty fallback policy for incompatible unmapped fields, and original-source-reference field behavior for updates.
...
6. Verify field mapping correctness on sampled items, including replaceTo remapped values, transformed fields, and skipped identity fields.
...
- Field mapping supports replaceTo value remapping before transform conversion.
